...
const CameraScreen = (props) => {
  ...
  if (!hasCameraPermissions || !hasAudioPermissions || !hasGalleryPermissions) {
    return (
        <View>
          <Text>Please allow all premissions</Text>
        </View>
    )
  }
    ...
    if (cameraRef) {
      try {
        ...
        if (videoRecordPromise) {
          ...
          navigation.navigate('SaveVideo', { source, sourceThumb, id: props.route.params.id, userEmail: props.route.params.userEmail })
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const pickFromGallery = async () => {
    ...
    if (!result.cancelled) {
      ...
      navigation.navigate('SaveVideo', { source: result.uri, sourceThumb, id: props.route.params.id, userEmail: props.route.params.userEmail });
    }
  }
  ...
  return (
    <View style={{flex: 1}}>
      ...
      <View style={styles.cancelBtnContainer}>
        ...
        >
          <Feather name="corner-up-left" size={36} color={'white'} />
        </TouchableOpacity>
      </View>
